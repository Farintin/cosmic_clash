import pygame
from .ui import display, colors
from . import controls, bullets
from .explode import *
import db
...
class Craft:
    ...
    def body_collision(self, body, explosions):
        collide = False
    ...
    def collide_indication(self, bg_obj):
        if self.collide:
            self.collided = True
            self.collide = False
    ...
    def load_bullets(self):
        ...
        for pt in self.mozzles:
            shoot += [ self.bullet( [self.pos[0] + pt[0], self.pos[1] + pt[1]], self.bullet_rad, self.bullet_power) ]
...
class Craft1(Craft):
    def __init__(self):
        self.image = pygame.image.load('game/src/img/craft/space_craft.png').convert_alpha()
