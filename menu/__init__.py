...
from game.src.ui import display, colors
import levels
from game.src.ui.dom import *
from game.src.ui import components
from game.src import controls as ctrls
import db
...
def run(bg, bg_alpha, Home, Levels):
    if Home:
        global Option
        Quit = Option = False
        # Buttons
        ...
        while Home:
            ...
            if Click:
                ...
                playBtn.onClick([handler1], (mx_up, my_up))[0]
                if playBtn.click:
                    Home = False
                    Levels = True
...
puffy = pygame.image.load('game/src/img/item/puff.png').convert_alpha()
rapid = pygame.image.load('game/src/img/item/rapid.png').convert_alpha()
rapidAR = rapid.get_width() / rapid.get_height()
magnet = pygame.image.load('game/src/img/item/magnet.png').convert_alpha()
magnetAR = magnet.get_width() / magnet.get_height()
shield = pygame.image.load('game/src/img/item/shield-sm.png').convert_alpha()
shieldAR = shield.get_width() / shield.get_height()
bubble = pygame.image.load('game/src/img/item/bubble-sm.png').convert_alpha()
bubbleAR = bubble.get_width() / bubble.get_height()
lifeGem = pygame.image.load('game/src/img/item/gem.png').convert_alpha()
lifeGemAR = lifeGem.get_width() / lifeGem.get_height()
unit = pygame.image.load('game/src/img/item/unit.png').convert_alpha()
